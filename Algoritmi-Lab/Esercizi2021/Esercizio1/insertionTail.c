...
double singleExperiment(int length, int maxInstances, char* algo){
	...
	return tFinal;			//ritorno il tempo medio necessario per l'algoritmo
	free(A);
}
...
void experiment(int minLength, int maxLength){
	...
	int s = seed;
	for(int i = minLength; i < maxLength; i=i+step){	//ciclo per avere esperimenti singoli per ogni alg con lo stesso numero di el, chead ogni iterazione incrementa
...
void merge(int* A, const int low, const int mid, const int high) {
	...
	int n2 = high-mid+1; //numero di elementi nel secondo dei nuovi array (parte destra dell'array originale)
...
void hybridSort(int* A, const int low, const int high) {
	if((high-low+1)>threshold){ 	//se l'array è più lungo di threshold richiamo hybridSort e poi merge
		int mid=(high+low)/2;
	        hybridSort(A,low,mid);
	        hybridSort(A,mid+1,high);
	        merge(A,low,mid,high);
	}
	else {				//quando l'array avrà lunghezza inferiore a threshold, userò la versione adattata d inssertionSort
...
void quickSort(int* A, int low, int high) {	//uso in MofTQ
	if(low < high) {
...
bool isSorted(int* A, int n) {		//controlla che l'array che torna sia effettivamente ordinato
	for (int i = 0; i < n-1; i++) {
