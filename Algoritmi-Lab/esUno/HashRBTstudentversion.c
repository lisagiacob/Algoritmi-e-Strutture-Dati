...
linkedListNode_t* linkedListSearch(linkedList_t* list, const int v) {
    ...
    return NULL;
    printf("errore in linkedListSearch");
}
...
void rbtInsertFixup(rbt_t* rbt, rbtNode_t* z) {
    while (strcmp(z->parent->color, "red") == 0) {
        ...
        if (z->parent == z->parent->parent->left) {
            ...
            if (strcm(y->color, "red") == 0) {                            //First case: if uncle of z is red, else it's either second or third case
...
bool isRbt(rbt_t* rbt) {
    rbtNode_t* x;
    x = rbt->root;
    if (strcmp(rbt->root->color, "black") !=0 && strcmp(rbt->root->color, "red") != 0) return false;
    else if (rbt->root->parent == rbt->nil && strcmp(rbt->root->color, "black") != 0) return false;
    else while (x->left != rbt->nil && x->right != rbt->nil) {
        if (strcmp(x->color, "red") == 0){
            if (sctrcmp(x->left->color, "black") !=0 || strcmp(x->right->color, "black") != 0)                // if one or both of a red node aren't black, it isnt an rbt
            return false;
        }
        if (strcmp(rbt->nil->color, "black") != 0) return false;
        else {
            if (x->right != rbt->nil){
                    rbt->root == x->right;
                    isRbt(rbt);
...
int rbtComputeBlackHeight(rbt_t* rbt, rbtNode_t* x) {
    ...
    while (x != rbt->nil) {
        if (strcmp(x->right->color, "black") == 0) h += 1;
        x = x->right;
    }
    return h;
}
